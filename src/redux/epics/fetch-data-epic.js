...
const fetchDataEpic = (action$) => {
  return action$.pipe(
    ofType(START_FETCH),
    mergeMap((action) => {
      const iex = `https://cloud.iexapis.com/v1/stock/${action.payload.queryTerm}/batch?types=logo,quote,company,news,chart&range=1m&last=10&token=${process.env.REACT_APP_IEX_KEY}`;
